...
const FilterSection = ({ filter, setFilter, handleExport }) => (
  <div className={styles.filterContainer}>
    ...
    <button onClick={() => handleExport('excel')} className={styles.exportButton}>
      <Download size={16} className="md:size-4" /> Excel
    </button>
    <button onClick={() => handleExport('csv')} className={`${styles.exportButton} ml-2`}>
      <Download size={16} className="md:size-4" /> CSV
    </button>
  </div>
);
...
const Income = () => {
  ...
  const handleExport = useCallback(async (format = 'excel') => {
    try {
      ...
      // Fallback for Excel if server route fails
      if (format === 'excel') {
        const exportData = filteredTransactions.map((t) => ({
          Date: new Date(t.date).toLocaleDateString(),
          Description: t.description,
