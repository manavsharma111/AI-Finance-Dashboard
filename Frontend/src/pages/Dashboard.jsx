...
import { ArrowDown, BarChart2, PiggyBank, Plus, TrendingDown, TrendingUp, Wallet, 
  PieChart as PieChartIcon, IndianRupee, ChevronUp, ChevronDown, ShoppingCart
 } from "lucide-react"
import {
  getTimeFrameRange,
...
const Dashboard = () => {
  // get refreshTransactions from the outlet 
  const {
  ...
  }, [currentTimeFrameData, timeFrame]); // graph make a/c to this data 
  ...
  const handleExport = (format = 'excel') => {
    try {
      const exportData = outletTransactions.map(t => ({
        id: t.id || t._id,
      ...
      const filename = `finance_report_${new Date().toISOString().slice(0, 10)}`;
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      console.error(" failed to add tansactions  ", e.response || e.message || e)
    }
    finally {
      setLoading(false)
    }
      ...
      <div className={dashboardStyles.summaryGrid} >
        ...
        <FinancialCard icon={
         ...
         value={`₹${Math.round(displaySavings).toLocaleString()}`}
          additionalContent={
            <div className='flex items-center gap-1 text-cyan-600 mt-2 text-xs'>
